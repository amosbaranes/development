      ...
      var dic_ = {"obj" : "AdvancedTabs", "atm": atm_.my_name, "fun": "add_tab", "params": {"tab_name": tab_name_}}
             $.post(atm_.activate_function_link_,
                  ...
                  function(dic){
                    ...
                    my_title_=tab_name_.charAt(0).toUpperCase() + tab_name_.slice(1);
                    atm.tabs[tab_id_] = new Tab(atm, my_name=tab_name_,my_title=my_title_,html=my_title_,tab_functions=null,id=tab_id_);
                    atm.tabs[tab_id_].btn.click();try{atm.set_active_tab(atm.tabs[tab_id_].btn)} catch(er){alert(er)}
                    var editor_=atm.editor;
                    var click_event = new Event("click", {bubbles: true});
                    try{editor_.main_menus["save"].btn.dispatchEvent(click_event);} catch (er){alert(er)}
                  }.bind(atm=atm_));
...
AdvancedTabsManager.prototype.setTabs = function()
{
  ...
  $.post(this.activate_function_link_,
      ...
      function(dic){
          ...
          for (id_ in result)
          {
            atm_.tabs[id_] = new Tab(this, my_name=result[id_]["tab_name"], my_title=result[id_]["tab_title"], html=result[id_]["tab_text"],
...
function Tab(parent, my_name, my_title, html, tab_functions, id)
{
 ...
 this.parent=parent;this.tab_name=my_name;this.tab_title=my_title;
