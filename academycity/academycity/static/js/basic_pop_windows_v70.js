...
function OrderStreamerWin(my_name_, win_name_, win_title_, use_id, tab_obj_, data_link, order_link, link_to_activate_obj_function)
{
  ...
  delta_content = document.createElement("td");tr1.appendChild(delta_content);
  delta_content.setAttribute("style", "vertical-align: top");
  ...
  this.order_nav.onclick = function (event){
    var elm = event.target; var dic = Orders.order_dic["dic"];
...
//  1 option (call/put) cost = (0.06 * 100) * 2 (one for enter one for exit)
  ...
  for(z in btns){var i=btns[z];eval("this.x"+i+" = document.createElement('button');this.x"+i+".innerHTML="+i+";this.order_nav.appendChild(this.x"+i+");")}
  ...
  input_sell_limit = document.createElement("input");this.order_limits.appendChild(input_sell_limit);
  input_sell_limit.setAttribute("size", 5); input_sell_limit.setAttribute("style", "text-align:right");
  input_sell_limit.onchange=function(event){
      var elm=event.target; var value=elm.value; Orders.order_dic["order"]["sell_limit"]=value;
    }
  this.order_limits.appendChild(document.createElement("br"));
  ...
  input_buy_limit = document.createElement("input");this.order_limits.appendChild(input_buy_limit);
  input_buy_limit.setAttribute("size", 5); input_buy_limit.setAttribute("style", "text-align:right");
  input_buy_limit.onchange=function(event){
      var elm=event.target; var value=elm.value; Orders.order_dic["order"]["buy_limit"]=value;
    }
...
OrderStreamerWin.prototype.set_win_nav_panel = function()
{
  ...
  if(this.is_nav_panel==true)
  {
     ...
     this.confirm_order.setAttribute("style", "width:10%;");
     ...
     this.execute_order.setAttribute("style", "width:10%;");
...
OrderStreamerWin.prototype.manage_orders = function()
{
 ...
 //alert(JSON.stringify(order))
  var cost = 0;
  var s="<thead><th></th><th>type</th><th>strike</th><th>price</th><th>Qt</th><th>total</th></thead>"
  for(type in order)
  {
    for(s_ in order[type])
    {
     try{
        ...
        s+="<tr><td onclick=Orders.remove_strategy_by_type('"+type+"') style='cursor:pointer'>-</td><td>"+type+"</td><td>"+k+"</td><td style='text-align:right'>"+two_decimal_points(p)+"</td><td style='text-align:right'>"+q+"</td><td style='text-align:right'>"+two_decimal_points(t)+"</td></tr>";
      } catch (er) {}
    }
  }
 cost=Math.round(100*cost)/100;
 s+="<tr><td></td><td></td><td></td><td></td><td></td><td style='text-align:right'>"+two_decimal_points(cost)+"</td></tr>"
 s = "<table>" + s + "</table>";
...
BFStrategy.prototype.prepare_bf_order_ = function(event)
{
 ...
 this.prepare_bf_order(dic={"type":type,"n": n,"q":10});
}
...
//alert(JSON.stringify(dic));
 ...
 var q_=1*q;var s_=ll[strikes[n]]["symbol"]
 order[type][s_] = {"k":strikes[n], "q": q_}
 n=1*n+1;q_=-2*q;s_=ll[strikes[n]]["symbol"];
 order[type][s_] = {"k":strikes[n], "q": q_}
 n=1*n+1;q_=1*q;s_=ll[strikes[n]]["symbol"]
 order[type][s_] = {"k":strikes[n], "q": q_}
...
VerticalStrategy.prototype.prepare_vertical_order_ = function(event)
{
 ...
 this.prepare_vertical_order(dic={"k0": ks_[0], "k1": ks_[1], "type": type, "q":10});
}

VerticalStrategy.prototype.prepare_vertical_order = function(dic=null)
{
 //alert(JSON.stringify(dic))
 var type = dic["type"]; var q=dic["q"];
