...
class CourseScheduleUser(TruncateTableMixin, models.Model):
    ...
    course_schedule = models.ForeignKey(CourseSchedule, on_delete=models.CASCADE, related_name='courses_course_schedules')
...
class ServiceAttendance(models.Model):
    ...
    service_calender = models.ForeignKey(ServiceCalenders, on_delete=models.CASCADE, related_name='service_attendance_services_calender')
...
class Contribution(models.Model):
    ...
    # contributor_name = models.CharField(max_length=128, unique=True)
    date = models.DateField(default=timezone.now)

    def __str__(self):
        return f"{self.contributor} - {self.contribution_category.category_name} - {self.amount} on {self.date}"
